...
{block name='product_tabs'}
    <div class="tabs product-tabs product-sections">
      {* <div id="product_infos_tabs" class="nav_2">
          {if $product.description}
              <div class="nav_item col-sm-auto">
                  <a class="nav_link{if $product.description} active{/if}"
                     href="#description">
                      {l s='Description' d='Shop.Theme.Catalog'}
                  </a>
              </div>
          {/if}
          <div class="nav_item col-sm-auto" id="product-details-tab-nav">
              <a class="nav_link{if !$product.description} active{/if}"
          ...
          {* {if $product.attachments}
              <div class="nav_item col-sm-auto">
                  <a class="nav_link" href="#attachments">
                      {l s='Attachments' d='Shop.Theme.Catalog'}
                  </a>
              </div>
          {/if} *}
          {* {hook h='displayProductTabTitle' product=$product }

          {if $iqitTheme.pp_accesories == 'tab'}
              {if $accessories}
                  <div class="nav_item col-sm-auto">
                      <a class="nav_link" href="#accessories-tab">
          ...
          {if isset($product_manufacturer) && $product_manufacturer->description != ''}
              <div class="nav_item col-sm-auto">
                  <a class="nav_link" href="#brand-tab">
          ...
          {foreach from=$product.extraContent item=extra key=extraKey}
              <div class="nav_item col-sm-auto">
                  <a class="nav_link"
                     href="#extra-{$extraKey}" {foreach $extra.attr as $key => $val} {$key}="nav_link-{$val}"{/foreach}> {$extra.title nofilter}</a>
              </div>
          {/foreach}
      </div>  *}
        {hook h='displayProductTabSection' product=$product show_tab_h=false}
        {* {if $product.description}
                        ...
                        <div class="products slick-products-carousel products-grid slick-default-carousel">
                            {foreach from=$accessories item="product_accessory" key="position"}
                                {block name='product_miniature'}
                                    {include file='catalog/_partials/miniatures/product.tpl' product=$product_accessory  carousel=true position=$position}
                                {/block}
                            {/foreach}
                        </div>
                    </div>
