...
class IqitElementorWidget_ProductsListTabs
{
    ...
    public function parseOptions($optionsSource, $preview = false)
    {
        ...
        foreach ($optionsSource['tabs'] as $tab){
            ...
            // start xiao
            if (strpos($source,'xc_') !== false) {
              $id_cat = str_replace('xc_','',$source);
              $parsedTab['cat_link'] = $this->context->link->getCategoryLink($id_cat);
            }
            elseif ($source=='pd') {
              $parsedTab['cat_link'] = $this->context->link->getPageLink("PricesDrop");
            }
            else {
              $parsedTab['cat_link'] = false;
            }
            // end xiao
