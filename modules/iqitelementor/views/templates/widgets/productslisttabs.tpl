...
<div class="tabs elementor-products-tabs">
    <ul class="nav nav-tabs">
        {foreach from=$tabs item="tab" name=productTabs}
            <li class="nav-item">
                <a class="nav-link{if $smarty.foreach.productTabs.first} active{/if}" data-toggle="tab" href="#ie-{$tab.uid}-ptab-{$smarty.foreach.productTabs.iteration}">{$tab.title}<i class="fas fa-chevron-down"></i></a>
            </li>
        {/foreach}
    </ul>
        ...
        {foreach from=$tabs item="tab" name=productTabs}
            <div class="tab-pane {if $smarty.foreach.productTabs.first} active{/if}" id="ie-{$tab.uid}-ptab-{$smarty.foreach.productTabs.iteration}">
                ...
                {/if}
                        ...
                        {/foreach}
                        {if $tab.cat_link}
                        <div class="product_list_tab_seemore">
                          <a href="{$tab.cat_link}">
