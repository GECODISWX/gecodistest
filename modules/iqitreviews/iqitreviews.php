...
class IqitReviews extends Module implements WidgetInterface
{
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        if (preg_match('/^displayProductExtraContent\d*$/', $hookName)) {
            ...
            $array[] = (new ProductExtraContent())
                ->setTitle('<span class="iqitreviews-title">'. $this->trans('Reviews', [], 'Modules.Iqitreviews.Iqitreviews') . '</span>')
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        if (preg_match('/^displayFooterProduct\d*$/', $hookName)) {
            $reviews = IqitProductReview::getByProductAndAsp((int)$configuration['product']['id_product']);
            $snippetData = IqitProductReview::getSnippetDataAndAsp((int)$configuration['product']['id_product']);
            ...
            $reviews = IqitProductReview::getByProductAndAsp((int)$configuration['product']->id);
            $snippetData = IqitProductReview::getSnippetDataAndAsp((int)$configuration['product']->id);
    ...
    public function hookDisplayHome($params){
      $lastest_reviews = IqitProductReview::getLatestReviewAndAsp(20);
      foreach ($lastest_reviews as $key => $review) {
        if (isset($review['id_product'])) {
          $id_product = $review['id_product'];
          $product_name = Product::getProductName($id_product);
          $cover = Image::getCover($review['id_product']);
          $lastest_reviews[$key]['cover_url'] = $this->context->link->getImageLink($product_name,$cover['id_image'],'small_default');
          $lastest_reviews[$key]['product_url'] =$this->context->link->getProductLink($id_product);
