...
<div id="ph_simpleblog_categories" class="block block-toggle block-categories block-links js-block-toggle">
	<h5 class="block-title"><span><a href="{ph_simpleblog::getLink()}" title="{l s='Blog' mod='ph_simpleblog'}">{l s='Blog' mod='ph_simpleblog'}</a></span> </h5>
			...
			{foreach $categories AS $category}
				<li><a href="{$category['url']}" title="{l s='Link to' mod='ph_simpleblog'} {$category['name']}">{$category['name']}</a>
					{if isset($category['childrens'])}
						...
						<span class="collapse-icons" data-toggle="collapse" data-target="#blog_subcategory_{$category['id']}">
